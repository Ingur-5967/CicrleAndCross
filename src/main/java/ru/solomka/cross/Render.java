...
public class Render {
    ...
    public static void renderMap(GameMap gameMap) {
        ...
        Map<Player, List<IntPair<Integer, Integer>>> busyCoordinates = gameMap.getBusyPositions();
        ...
        for (int y = 0; y < gameMap.getGameSetting().getMapSize()[1]; y++) {
            ...
            for(int x = 0; x < gameMap.getGameSetting().getMapSize()[0]; x++) {
                ...
                for(Map.Entry<Player, List<IntPair<Integer, Integer>>> aMap : busyCoordinates.entrySet())
